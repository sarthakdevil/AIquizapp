...
export const fetchQuizzes = () => async (dispatch) => {
  ...
  try {
    ...
    // Use quiz data directly from database without any normalization
    const quizzes = response.data.map((quiz) => ({
      id: quiz._id,
      quizName: quiz.quiz_name,
      questionsAndAnswers: quiz.questions_and_answers, // Use database data as-is
      questionCount: quiz.questions_and_answers?.length || 0,
      quizTime: quiz.quiz_time || 0,
